...
local Toggle = GrabTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            masslessGrabCoroutine = coroutine.create(function()
                while true do
                    local child = workspace:FindFirstChild("GrabParts")
                    if child then
                        local grabPart = child:FindFirstChild("GrabPart")
            ...
            coroutine.resume(masslessGrabCoroutine)
        else
            if masslessGrabCoroutine then
                coroutine.close(masslessGrabCoroutine)
                masslessGrabCoroutine = nil
            end
        end
    end,
})
 ...
 local Toggle = GrabTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            spinGrabCoroutine = coroutine.create(function()
                while true do
                    local child = workspace:FindFirstChild("GrabParts")
                    if child then
                        local grabPart = child:FindFirstChild("GrabPart")
            ...
            coroutine.resume(spinGrabCoroutine)
        else
            if spinGrabCoroutine then
                coroutine.close(spinGrabCoroutine)
                spinGrabCoroutine = nil
            end
        end
    end,
})
            ...
            killGrabCoroutine = coroutine.create(function()
                while true do
                    ...
                    if child then
                        ...
                        if head then
                            while workspace:FindFirstChild("GrabParts") do
                                head.Position = Vector3.new(0, -1000, 0)
                                wait()
                            end
                        end
                    end
...
local Toggle = GrabTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            ragdollGrabCoroutine = coroutine.create(function()
                while true do
                    local child = workspace:FindFirstChild("GrabParts")
                    if child then
                        local grabPart = child:FindFirstChild("GrabPart")
                        local grabbedPart = grabPart:FindFirstChild("WeldConstraint").Part1
                        local humanoid = grabbedPart.Parent:FindFirstChild("Humanoid")
                        if humanoid then
            ...
            coroutine.resume(ragdollGrabCoroutine)
        else
            if ragdollGrabCoroutine then
                coroutine.close(ragdollGrabCoroutine)
                ragdollGrabCoroutine = nil
            end
        end
    end,
})
...
local Toggle = GrabTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            doubleGrabCoroutine = coroutine.create(function()
                while true do
                    local child = workspace:FindFirstChild("GrabParts")
                    if child then
                        local grabPart = child:FindFirstChild("GrabPart")
                        ...
                        if head then
                            while workspace:FindFirstChild("GrabParts") do
                                CreateLine:FireServer(head, grabPart.Position)
                                wait(0.1)
                            end
            ...
            coroutine.resume(doubleGrabCoroutine)
        else
            if doubleGrabCoroutine then
                coroutine.close(doubleGrabCoroutine)
                doubleGrabCoroutine = nil
            end
        end
    end,
})
        ...
        if enabled then
            infJumpConn = UserInputService.JumpRequest:Connect(function()
                if playerCharacter then
                    playerCharacter:FindFirstChildOfClass("Humanoid"):ChangeState("Jumping")
                end
            end)
        else
            if infJumpConn then
...
local Toggle = ObjectGrabTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            bangGrabCoroutine = coroutine.create(function()
                while true do
                    local child = workspace:FindFirstChild("GrabParts")
                    if child then
                        local grabPart = child:FindFirstChild("GrabPart")
                        local grabbedPart = grabPart:FindFirstChild("WeldConstraint").Part1
                        if grabbedPart then
                            grabbedPart.Orientation = Vector3.new(math.random(-180,180), math.random(-180,180), math.random(-180,180))
                            wait(0.05)
                        end
                    end
                    wait()
            ...
            coroutine.resume(bangGrabCoroutine)
        else
            if bangGrabCoroutine then
                coroutine.close(bangGrabCoroutine)
                bangGrabCoroutine = nil
            end
        end
    end,
})
...
local Toggle = DefanseTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            silentKickCoroutine = coroutine.create(function()
                while true do
                    local character = localPlayer.Character
                    if character and character:FindFirstChild("Head") then
                        local head = character.Head
            ...
            coroutine.resume(silentKickCoroutine)
        else
            if silentKickCoroutine then
                coroutine.close(silentKickCoroutine)
                silentKickCoroutine = nil
            end
        end
    end,
})
